...
const BACKEND: &str = "indexmap";
#[cfg(not(feature = "dictionary-indexmap"))]
const BACKEND: &str = "btree";

fn bench_name(name: &str) -> String {
    format!("{name}_{BACKEND}")
}

fn parse_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group(format!("parse_{BACKEND}"));
...
fn parse_filtered_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group(format!("parse_filtered_{BACKEND}"));
...
fn backend_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group(format!("backend_{BACKEND}"));
